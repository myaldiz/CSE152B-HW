# CSE152B - Spring 2025: Homework 2
## Computing resources
...
- Launch a GPU instance with (1 GPU, 4 CPU, 16G RAM). This will lead you to a Jupyter Notebook GUI, where you can browse files, launch a terminal with bash environment via (upper-right) New -> Terminal.
- You can also access the container with command line from your local terminal (see README.md for more).

## Instructions for setting up the environment

The dataset is originally from the [KITTI Odometry dataset](http://www.cvlibs.net/datasets/kitti/eval_odometry.php). 
You can find the processed version in the `~/public/dataset_SfM` folder in datahub cluster.
To work with the dataset, you need to create a symbolic link to the dataset folder in your working directory.
In addition to the HW1 environment, we need to install one more package to the local environment.
Here are the steps to set up the environment:
```bash
cd CSE152B-HW/HW2/
# Create a symbolic link to the dataset folder
ln -s ~/public/dataset_SfM .
# We need to install SWIG for pyviso
# It is already installed in the container, but if you want to install it manually, you can do so with:
# conda install swig
cd ./pyviso/src
pip install -e .
```
...
The homework is in the Jupyter Notebook ``HW2.ipynb``.
...
![](images/demo.png)
Because you don't have access to an active X graphics server on JupyterHub, you are not going to see a Linux GUI like above. Hoeever, you can open the images from the file explorer within Jupyter Notebook, or fetch the files you can use commands like `scp` to transfer files from the cluster to your local machine:

From your local machine: 

``scp -r <USERNAME>@dsmlp-login.ucsd.edu:{path to /pyviso2/vis} {LOCAL PATH}``
